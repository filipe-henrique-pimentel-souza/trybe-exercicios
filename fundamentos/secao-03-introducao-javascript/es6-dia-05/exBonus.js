...
const substituaX = (nomeRecebido) => {
    const frase = 'Tryber x aqui!';
    return frase.replace(/x/g, nomeRecebido)
}

const substitucao = substituaX('Beteto')
const minhasSkills = (retorno) => {
    const skills = ['JavaScript', 'CSS', 'HTML'];
    let retornado = `${retorno}\n\nMinhas três principais habilidades são:\n\n`
    for (const i of skills) {
        retornado += `-${i}\n`
    }
    return retornado
}
console.log(minhasSkills(substitucao))
